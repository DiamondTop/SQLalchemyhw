"""
This python script uses flask and SQL alchemy to simulate API requests of weather data of WAIHEE, HI.
"""
...
#save table references for measurement & station table
...
def precipitation():
    ...
    final_date_query = session.query(func.max(func.strftime("%Y-%m-%d", measurements.date))).all()
    ...
    precip_data = session.query(func.strftime("%Y-%m-%d", measurements.date), measurements.prcp).\
